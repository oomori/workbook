    ...
    if (sqlite3_open([path UTF8String], &database) == SQLITE_OK) {
        ...
        if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
				...
				word.lang =	@((char *)sqlite3_column_text(statement, 1));
				word.lemma =@((char *)sqlite3_column_text(statement, 2));
				//word.pron = [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 3)];
				word.pos =	@((char *)sqlite3_column_text(statement, 3));
    ...
    if (sqlite3_open([path UTF8String], &database) == SQLITE_OK) {
		...
		if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) == SQLITE_OK) {
			...
			while (sqlite3_step(statement) == SQLITE_ROW) {
				...
				word.lemma =		@((char *)sqlite3_column_text(statement, 2));
				//word.pron =		[NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 3)];
				word.pos =		@((char *)sqlite3_column_text(statement, 4));
				[retArry addObject:word];
			}
		}else{
			//データベースが開けなかったとき
		...
		for (i = 0; i < count; i++) {
			...
			if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) == SQLITE_OK) {
				...
				while (sqlite3_step(statement) == SQLITE_ROW) {
					...
					sense.synset = @((char *)sqlite3_column_text(statement, 0));
					//sense.lexid = sqlite3_column_int(statement, 1);//
					sense.lang = @((char *)sqlite3_column_text(statement, 1));
		...
		for (i = 0; i < count; i++) {
			...
			if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) == SQLITE_OK) {
				...
				while (sqlite3_step(statement) == SQLITE_ROW) {
					...
					NSString *str = @((char *)sqlite3_column_text(statement, 1));
    ...
    if (sqlite3_open([path UTF8String], &database) == SQLITE_OK) {
		...
		if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) == SQLITE_OK) {
			...
			while (sqlite3_step(statement) == SQLITE_ROW) {
				...
				synlink.synset1 =	@((char *)sqlite3_column_text(statement, 0));
				synlink.synset2 =	@((char *)sqlite3_column_text(statement, 1));
				synlink.link =		@((char *)sqlite3_column_text(statement, 2));
				synlink.src =		@((char *)sqlite3_column_text(statement, 3));
    ...
    if (sqlite3_open([path UTF8String], &database) == SQLITE_OK) {
        ...
        if (sqlite3_prepare_v2(database, sql, -1, &statement, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
				...
				synset.synset =	@((char *)sqlite3_column_text(statement, 0));
				synset.pos =		@((char *)sqlite3_column_text(statement, 2));
				//synset.name =		[NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 3)];
				synset.src =		@((char *)sqlite3_column_text(statement, 1));
