...
-(void)method001
{
	...
	NSDictionary *dic = [[NSDictionary alloc] initWithObjects:objArr forKeys:keyArr];
    
	//辞書から作る
	NSEnumerator *aKeyEnumerator = [ dic keyEnumerator ];
	
	id objects = [aKeyEnumerator allObjects];
	NSLog(@"allObjects %@",[objects description]);
	
	//一旦allObjectsを呼び出すとaKeyEnumeratorは空になっている
	NSLog(@"allObjects %@",[[aKeyEnumerator allObjects] description]);
}
